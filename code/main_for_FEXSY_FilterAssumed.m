...
%A FEXSY experiment is simulated.
%At t=0, ONLY INTRACELLULAR WATER IS CONSIDERED, assuming a
%filter was already applied and supressed extraceullar magnetization.
...
%%%%%%%%% FEXSY %%%%%%%%%%%%%%%%%%
g1_user = 250;                           % filter block gradient pulse strengh g/cm
...
%%%%%%%%%%%%%%%% For the PGSE experiment (for normalization) -- spination starts at 0  %%%%%%%%%%%%%%%%%
...
x0f=zeros(spinsnum,1);y0f=zeros(spinsnum,1);z0f=zeros(spinsnum,1);indicator0f=zeros(spinsnum,1); indicatorf=zeros(spinsnum,1);  sps=zeros(spheresnum,1);xfin=zeros(spinsnum,1);yfin=zeros(spinsnum,1);zfin=zeros(spinsnum,1);
x0=zeros(spinsnum,1);y0=zeros(spinsnum,1);z0=zeros(spinsnum,1);indicator0=zeros(spinsnum,1); indicator=zeros(spinsnum,1);
spinsnumi0=0;spinsnume0=0;spinsnumi=0;spinsnume=0;
E=zeros(length(q2),N); E1=zeros(length(q2),1); E2=zeros(length(q2),N-1); 

%%%%%%% PGSE experiment for normalization.
parfor s=1:spinsnum
    %%%
    [x0f(s),y0f(s),z0f(s),indicator0f(s),spheretrackeri,sps] = CreateSpins(FOVx,FOVy,FOVz,R,spheresnum,xcenter,ycenter,zcenter,central,spination,ck,s,scale); %Create Spins according the Spination
    %%%

    x = x0f(s); y = y0f(s); z = z0f(s); indicatorf(s)=indicator0f(s); 
    loopE=zeros(length(q2),1);
    
   i=0;       
    while dt*i < tau+dt    
    %Move the spin
    [sps,x,y,z,indicatorf(s),spheretrackeri] = MovingSpins(kappa,R,dre,dri,dt,De,Di,FOVx,FOVy,FOVz,xcenter,ycenter,zcenter,sps,spheresnum,x,y,z,indicatorf(s),spheretrackeri); 

    i=i+1
    end
    
    for j=1:length(q2)
         loopE(j,1) = exp( - 1i * q2(j) * (x-x0f(s)) );         
    end
...
E(:,1)=E1(:,1);
inef=find(indicatorf==2);

fe=length(inef)/spinsnum;

%%%%%%%%%% Assuming a filter was applied and killed all extra-cellular magnetization
spination=2;sspss=spinsnum;

parfor s=1:spinsnum

    %%%
    [x0(s),y0(s),z0(s),indicator0(s),spheretrackeri,sps] = CreateSpins(FOVx,FOVy,FOVz,R,spheresnum,xcenter,ycenter,zcenter,central,spination,ck,s,scale); %Create Spins according the Spination
    %%%

    x = x0(s); y = y0(s); z = z0(s); indicator(s)=indicator0(s); 
    loopE=zeros(length(q2),N-1);
    %%%%%%%%%%%%%%%%%%%%%%%%%Finally: ITERATIONS
   ...
   [sps,x,y,z,indicator(s),spheretrackeri] = MovingSpins(kappa,R,dre,dri,dt,De,Di,FOVx,FOVy,FOVz,xcenter,ycenter,zcenter,sps,spheresnum,x,y,z,indicator(s),spheretrackeri); 
          ...
          if dt*k == Tm(counter)    %Collect data at points Tm to create the attenuation curve later
             f=0;   

                     xP=x; yP=y; zP=z; indicatorp=indicator(s); spheretrackerip=spheretrackeri; %Remember the location after Tm(Counter), 
                     % we will now start a measruement block. After the
                     % measurement block ends, we move the spin back to
                     % this location and continue the clock from
                     % Tm(counter), instead of starting over.
                    ...
                    for j=1:length(q2)
                        loopE(j,counter) = loopE(j,counter) + exp(-1i *q2(j)*(x-xP)) ;                
                    end    
...
for count = 1:N
    Enorm(:,count) = abs(E(:,count)/E(1,count));  
end
...
xlabel('\boldmath$\tilde{q}^2\Delta \hspace{1ex} [s/m^2]$','Interpreter','latex')
...
ylabel('\textbf{Normalized ADC}','interpreter','Latex')
xlabel('\boldmath$t_m \hspace{1ex} [ms]$','interpreter','Latex')
