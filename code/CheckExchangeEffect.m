...
ki = 1/100;                                 %The "real" exchange rate from intra to extra
...
% figure(4)
% g_an = linspace(1,500,600);    
% g_an = g_an*(10^(-8));            %T/micrometer 
% q_an = 2 * pi * gyro * g_an * d;       %q vector, rad/micrometer
% 
% analytical =  (abs(3*(q_an*R .* cos(q_an*R) - sin(q_an*R))./((q_an*R).^3))).^2;
% plot(q_an*R/2/pi,log(analytical),'-b','MarkerSize',20,'LineWidth',3)
% hold on
...
fo = fitoptions('Method','NonlinearLeastSquares',...
               ...
               'Upper',[10,1,inf],...
               'StartPoint',[2 0.7 2]);
ft = fittype('(1-f1)*((abs(3*(x*R1 .* cos(x*R1) - sin(x*R1))/((x*R1)^3)))^2) + f1*exp(-(x^2)*dntau) ','coefficients',{'R1','f1','dntau'},'options',fo); % Define ft as linear fit

[fitobject,gof] = fit(q',E,ft);


dntau=fitobject.dntau
R1=fitobject.R1
f1=fitobject.f1
dntau/tau

fit1 = (1-f1)*((abs(3*(q*R1 .* cos(q*R1) - sin(q*R1))./((q*R1).^3))).^2) + f1*exp(-q.^2*dntau);
hold on
plot(q*R/2/pi,log((E)),'.k','MarkerSize',20,'LineWidth',3)
hold on
plot(q*R/2/pi,log((fit1)),'.b','MarkerSize',20,'LineWidth',3)
ylabel('$E$','Interpreter','latex')
xlabel('$\tilde{q}R$','Interpreter','latex')
