...
E=zeros(NDELTA+1,Ng); sdis=zeros(NDELTA+1,1);
...
parfor s=1:spinsnum
      ...
      %%%%%%%%%%%%%%%%%%%%%%%%%Finally: ITERATIONS
      counter=1; loopsdis=zeros(length(DELTA),1); loopE=zeros(length(DELTA),Ng);
      for i=0:N
    ...
    %                      
                            ...
                            if indicator(s)==1
                                loopE(counter,j) = loopE(counter,j) .* exp( - tau/T2i );
                            elseif indicator(s)==2    
                                loopE(counter,j) = loopE(counter,j) .* exp( - tau/T2e);
                            end   
                    end
                    counter=counter+1;
