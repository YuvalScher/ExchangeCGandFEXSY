...
%filter was applied and supressed extraceullar magnetization)
...
%The direction will be chosen randomly in each iteration (see MovingSpins)dre=sqrt(6*De*dt);
dri=sqrt(6*Di*dt);
...
LOOP=30;
for  i=1:length(Tm)
EEE((LOOP*(i-1)+1):(i*LOOP))=abs(E(:,i+1));
